...
function AnalyticsPage() {
  ...
  const handleExportAnalytics = () => {
    const csvContent = [
      ...
      `"${region}","Total Non-Compliant Products",12482,"${timeRange}"`,
      `"${region}","Repeat Offenders",482,"${timeRange}"`,
      `"${region}","Compliant Clearance Rate",88.4%,"${timeRange}"`,
      `"Maharashtra","Non-Compliance Rate",18.2%,"${timeRange}"`,
      `"Delhi NCR","Non-Compliance Rate",14.5%,"${timeRange}"`,
      `"Karnataka","Non-Compliance Rate",11.1%,"${timeRange}"`,
      `"Gujarat","Non-Compliance Rate",8.4%,"${timeRange}"`,
      `"Tamil Nadu","Non-Compliance Rate",5.2%,"${timeRange}"`,
    ].join('\n');
    ...
    link.href = url;
    link.setAttribute('download', `METROSCAN_ANALYTICS_${region.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-3">
          <select 
            ...
            className="bg-card border border-border-subtle rounded-lg px-4 py-2 text-sm font-bold shadow-sm focus:outline-none focus:border-primary text-text-secondary cursor-pointer"
          >
          ...
          <button 
            ...
            className="px-4 py-2 bg-primary text-white text-sm font-bold rounded-lg shadow-sm hover:bg-primary-dark transition-colors flex items-center gap-2 cursor-pointer"
          >
